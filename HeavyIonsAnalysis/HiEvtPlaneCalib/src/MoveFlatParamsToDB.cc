...
MoveFlatParamsToDB::MoveFlatParamsToDB(const edm::ParameterSet& iConfig):
  rescorloc_ ( iConfig.getParameter<std::string>("rescorloc") ),
  outtag_ ( iConfig.getParameter<std::string>("outtag") ),
  infile_ ( iConfig.getParameter<std::string>("infile") )
{
  rpFlat = new RPFlatParams();
  cout<<"Enter MoveFlatParamsToDB"<<endl;
  //now do what ever initialization is needed
  inFile = new TFile(infile_.data());
  if(inFile->IsZombie()) {
  ...
  nRP = cnt;
  cout<<"nRP = "<<nRP<<endl;
  FILE * ftest = fopen(rescorloc_.data(),"r");
  if(ftest) {
...
bool MoveFlatParamsToDB::GetRescor(double bin, string epname, double * restmp, double * erestmp) {
  FILE * ftest;
  string resname=Form("%s/%s_%d.dat",rescorloc_.data(),epname.data(),(int)bin);
  ftest = fopen(resname.data(),"r");
  if(ftest) {
    cout<<Form("%s added",resname.data())<<endl;
  }  else {
    std::cout<<"Resolution corrections not found: "<<resname<<std::endl;
    return kFALSE;
  }
  int nbins = 0;
  char buf[80];
...
MoveFlatParamsToDB::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(pool.isAvailable()){
    if(pool->isNewTagRequest(outtag_.data()) ) {
      pool->createNewIOV<RPFlatParams>(rpFlat,pool->beginOfTime(), pool->endOfTime(),outtag_.data());
    } else {
      pool->appendSinceTime<RPFlatParams>(rpFlat, pool->currentTime(),outtag_.data());
    }
  }
}
